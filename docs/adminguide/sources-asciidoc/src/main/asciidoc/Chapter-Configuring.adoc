...
| exposureLayerData | Data from Exposure Layer. Colon separated values presented in following order: messageId, correlationId, elApiStart (time of http request), alApiStop (time of http response), elQueStart (time when message is placed in queue), elQueStop (time when message is taken from queue).
|===
...
http://127.0.0.1:8080/{this-httpprefix}/sendSms?userid=user1&password=password&msg=exampleMessage_03&format=json&smscEncoding=UCS2&messageBodyEncoding=UTF16&sender=4321&to=6666,6667&senderTon=5&senderNpi=0&udh=%06%05%04%13%01%13%01&exposureLayerData=123456:112233:1503655696096:1503655721872:1503655736078:1503655741991
----
