...
public class CdrFinalGenerator {
    ...
    public static void generateFinalCdr(Sms smsEvent, String status, String reason, boolean generateReceiptCdr, boolean generateCdr,
       ...
       /* String timestampA = null, timestampB = null, timestampC = null;
        ...
        if(exposureLayerDataSet == true){
            ...
            if(gw_que_start !=0 || gw_que_stop != 0){
                T7 = gw_out_stop - el_api_start - (gw_out_stop - gw_que_start) - (el_que_stop - el_que_start);
            }else{
                T7 = gw_out_stop - el_api_start - (el_que_stop - el_que_start);
            }
        }else {
            T1 = gw_out_stop - gw_inc_start;
            if(gw_que_start !=0 || gw_que_stop != 0){
                T7 = gw_out_stop - gw_inc_start - (gw_out_stop - gw_que_start);
            }else{
                T7 = gw_out_stop - gw_inc_start;
            }
        }

        if(gw_que_start !=0 || gw_que_stop != 0){
            T9 = el_que_stop - el_que_start;
            T10 = gw_out_stop - gw_que_start;
        }else{
            T9 = 0;
            T10 = 0;
        }

        T2 = el_api_stop - el_api_start;
        T3 = gw_out_stop - gw_out_start;
        T4 = oc_dia_stop - oc_dia_start;
        T5 = gw_inc_stop - gw_inc_start;
        T6 = el_api_stop - el_api_start;
        T8 = T1 - T7;
...
//        String first20CharOfSms = new String(rawSms);
   ...
   /* public static void generateFinaldCdr(boolean mcDeliveryReceipt, String shortMessageText, TlvSet tlvSet, Long tsA,
                                           Long tsB, Long tsC, Long messageId, String origEsmeName, EventType eventType, ErrorCode errorCode,
                                           String messageType, long statusCode, int mprocRuleId, String sourceAddrAndPort, String destAddrAndPort,
