...
public class SchedulerResourceAdaptor implements ResourceAdaptor {
    ...
    public void eventProcessingSuccessful(ActivityHandle activityHandle, FireableEventType arg1, Object arg2, Address arg3,
            ReceivableService arg4, int flags) {
        if (this.tracer.isFineEnabled()) {
    ...
    public void raVerifyConfiguration(ConfigProperties properties) throws InvalidConfigurationException {
        ...
        configProperty = properties.getProperty(CONF_FETCH_MAX_ROWS);
        if (configProperty == null || configProperty.getValue() == null || !(configProperty.getValue() instanceof Integer)
                || ((Integer) configProperty.getValue()) <= 0) {
            throw new InvalidConfigurationException("Wrong value of '" + CONF_FETCH_MAX_ROWS + "' property: " + configProperty);
        }
        ...
        if (configProperty == null || configProperty.getValue() == null || !(configProperty.getValue() instanceof Long)
            ...
            throw new InvalidConfigurationException("Wrong value of '" + CONF_FETCH_PERIOD + "' property: " + configProperty);
        }

        configProperty = properties.getProperty(CONF_HIGH_WATER_MARK);
        if (configProperty == null || configProperty.getValue() == null || !(configProperty.getValue() instanceof Integer)
                || ((Integer) configProperty.getValue()) <= 0) {
            throw new InvalidConfigurationException("Wrong value of '" + CONF_HIGH_WATER_MARK + "' property: " + configProperty);
        }
